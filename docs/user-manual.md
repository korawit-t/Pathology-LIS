...
### 1.6 การยืนยันตัวตนสองขั้น (Two-Factor Authentication)

ฟีเจอร์นี้เพิ่มขั้นตอนที่สองตอนเข้าสู่ระบบ นอกจากรหัสผ่านแล้วต้องกรอกเลข 6 หลัก
จากแอปในมือถือด้วย เพื่อว่ารหัสผ่านที่รั่วไปเพียงอย่างเดียวจะเข้าระบบไม่ได้

> ระบบเปิดใช้ฟีเจอร์นี้หรือไม่ขึ้นกับการตั้งค่าของแต่ละที่ ถ้ายังไม่ได้เปิด
> คุณยังตั้งค่าไว้ล่วงหน้าได้ แต่จะยังไม่ถูกถามเลขตอนเข้าสู่ระบบ

#### เตรียมแอป

ติดตั้งแอปตัวใดตัวหนึ่งบนมือถือ ทั้งหมดฟรีและใช้แทนกันได้:

- **Google Authenticator** (Android / iOS)
- **Microsoft Authenticator** (Android / iOS)
- **FreeOTP** (Android / iOS)

แอปเหล่านี้ทำงานโดยไม่ต้องต่ออินเทอร์เน็ต เพราะคำนวณเลขจากเวลาในเครื่อง

#### การตั้งค่าครั้งแรก

1. คลิกไอคอนผู้ใช้งานมุมขวาบน เลือก **Two-Factor Authentication**
2. กด **Set up two-factor authentication**
3. กรอกรหัสผ่านของคุณอีกครั้ง (เพื่อยืนยันว่าเป็นตัวคุณจริง ไม่ใช่คนที่มานั่งใช้เครื่องต่อ)
4. เปิดแอปในมือถือ กดเพิ่มบัญชี แล้วสแกน QR code บนหน้าจอ
   - ถ้ากล้องใช้ไม่ได้ ให้เลือกเพิ่มแบบพิมพ์เอง แล้วพิมพ์ชุดตัวอักษรที่แสดงใต้ QR
5. กรอกเลข 6 หลักที่แอปแสดง แล้วกด **Confirm**

เสร็จแล้วครั้งต่อไปที่เข้าสู่ระบบจะถูกถามเลขนี้

> ⚠️ **ระบบนี้ไม่มีรหัสสำรองแบบพิมพ์เก็บไว้** เพราะกระดาษที่เก็บรหัสมักจบลงที่
> ลิ้นชักหรือแปะข้างจอ ซึ่งทำให้การมีขั้นตอนที่สองไม่มีความหมาย
> ถ้าทำมือถือหาย ดูหัวข้อ **1.6.4**

#### 1.6.1 การเข้าสู่ระบบหลังตั้งค่าแล้ว

1. กรอก Username และ Password ตามปกติ
2. หน้าจอจะเปลี่ยนเป็นช่องกรอกเลข 6 หลัก — เปิดแอปแล้วอ่านเลขปัจจุบัน
3. กด **Verify**

เลขในแอปเปลี่ยนทุก 30 วินาที ถ้าใกล้หมดเวลาให้รอชุดถัดไปแล้วค่อยกรอก
และ**เลขชุดเดิมใช้ซ้ำไม่ได้** ถ้าเพิ่งใช้ไปให้รอชุดใหม่

#### 1.6.2 "จำอุปกรณ์นี้" (Remember this device)

ในหน้ากรอกเลขจะมีช่องให้ติ๊ก ถ้าติ๊กไว้ เบราว์เซอร์เครื่องนี้จะไม่ถูกถามเลข
อีกตามจำนวนวันที่ผู้ดูแลระบบตั้งไว้ (ค่าเริ่มต้น 14 วัน)

**ควรติ๊ก** — เครื่องคอมพิวเตอร์ส่วนตัวของคุณเอง

**ไม่ควรติ๊ก** — เครื่องส่วนกลางในวอร์ด เครื่องที่คนอื่นใช้ร่วม หรือเครื่องที่ยืมเขามาใช้ชั่วคราว

ดูรายการเครื่องที่จำไว้ทั้งหมดได้ที่เมนู **Two-Factor Authentication**
ถ้าเจอเครื่องที่จำไม่ได้ว่าเคยใช้ ให้กดลบทิ้งทันที แล้วแจ้งผู้ดูแลระบบ —
รายการนี้เป็นที่เดียวที่จะเห็นว่ามีใครแอบเข้าใช้บัญชีคุณอยู่หรือไม่

#### 1.6.3 ทำไมบางครั้งถูกถามเลขอีกทั้งที่เพิ่งเข้าระบบ

**ค่าเริ่มต้นคือไม่ถาม** ถ้าโรงพยาบาลของคุณไม่ได้ตั้งค่านี้ไว้ จะเซ็นออกรายงานได้เลย
ไม่มีอะไรมาขัด — ข้ามหัวข้อนี้ไปได้

แต่งานบางอย่างย้อนกลับไม่ได้ ผู้ดูแลระบบจึง**ตั้งให้ถามยืนยันตัวตนซ้ำก่อนทำได้** แม้จะติ๊ก
"จำอุปกรณ์นี้" ไว้แล้วก็ตาม งานที่เข้าข่ายคือ:

- การ**เซ็นออกรายงาน** (Surgical / Gyne / Non-Gyne)
- การแก้ไขผลที่อนุมัติไปแล้ว
- การเปลี่ยนการตั้งค่าระบบ

ตรงนี้กรอก**เลขจากแอป หรือรหัสผ่าน**ก็ได้ อย่างใดอย่างหนึ่ง

เมื่อยืนยันแล้วจะไม่ถูกถามอีกจนกว่าจะครบเวลาที่ผู้ดูแลระบบตั้งไว้ — **นับเป็นรอบการใช้งาน
ไม่ใช่นับรายเคส** เช่นตั้งไว้ 30 นาที ก็คือถามครั้งเดียวตอนเริ่มนั่งออกผล แล้วเงียบยาว
ถ้ารู้สึกว่าถูกถามบ่อยเกินไป ให้แจ้งผู้ดูแลระบบ ค่านี้ปรับได้ที่
Admin → System Settings → Security

#### 1.6.4 ทำมือถือหาย หรือเปลี่ยนเครื่องใหม่

**ถ้ามีผู้ดูแลระบบคนอื่นในหน่วยงาน** — ติดต่อผู้ดูแลระบบเพื่อขอรีเซ็ต
ผู้ดูแลจะยืนยันตัวตนของคุณก่อน แล้วล้างการตั้งค่าเดิมออก จากนั้นคุณเข้าสู่ระบบ
ด้วยรหัสผ่านอย่างเดียวได้ แล้วตั้งค่าใหม่ตามหัวข้อด้านบน

**ถ้าคุณเป็นผู้ดูแลระบบเพียงคนเดียว** — จะไม่มีใครให้ติดต่อ และเข้าระบบไม่ได้ด้วย
ต้องกู้คืนจากหน้าเครื่อง server โดยตรง ดู `docs/mfa-setup.md` หัวข้อ
*When nobody can sign in at all* คำสั่งคือ:

```
python scripts/reset_mfa.py <ชื่อผู้ใช้>
```

> ⚠️ **ก่อนเปิดใช้ฟีเจอร์นี้ ควรทดลองรันคำสั่งข้างต้นให้สำเร็จอย่างน้อยหนึ่งครั้ง**
> ถ้าคุณเป็นผู้ดูแลระบบคนเดียว นี่คือทางกลับเข้าระบบทางเดียวที่มี
> การมารู้ว่ามันใช้ไม่ได้ตอนที่เข้าระบบไม่ได้แล้ว คือสถานการณ์ที่ควรเลี่ยงที่สุด

**เปลี่ยนมือถือใหม่โดยที่เครื่องเก่ายังอยู่** — เปิดแอปบนเครื่องใหม่แล้วสแกน QR ใหม่
โดยเข้าเมนู Two-Factor Authentication → ปิดการใช้งานก่อน แล้วตั้งค่าใหม่

#### 1.6.5 ถ้าหน่วยงานกำหนดให้ต้องใช้

ผู้ดูแลระบบอาจกำหนดว่าบางบทบาทต้องตั้งค่าการยืนยันสองขั้น โดยมีช่วงผ่อนผันให้ก่อน

**ระหว่างช่วงผ่อนผัน** — เข้าใช้งานได้ตามปกติทุกอย่าง เมนู Two-Factor Authentication
จะแสดงว่าเหลืออีกกี่วัน

**เมื่อครบกำหนดแล้วยังไม่ได้ตั้งค่า** — ยังเข้าสู่ระบบได้ (ไม่ได้ถูกล็อกออกจากระบบ)
แต่ระบบจะพาไปหน้าตั้งค่าทันที และ**จะเซ็นออกรายงานหรือแก้การตั้งค่าไม่ได้จนกว่าจะตั้งค่าเสร็จ**

การตั้งค่าใช้เวลาไม่ถึงสองนาที ทำตามหัวข้อ **1.6** ด้านบนได้เลย

> ระบบไม่ล็อกคุณออกจากระบบทั้งหมดโดยตั้งใจ — ถ้าล็อกออกไปเลยคุณจะเข้าหน้าตั้งค่าไม่ได้
> ซึ่งเป็นหน้าที่ระบบกำลังบอกให้คุณไปทำพอดี

#### 1.6.6 ปัญหาที่พบบ่อย

| อาการ | สาเหตุที่เป็นไปได้ | วิธีแก้ |
|---|---|---|
| กรอกเลขถูกแล้วแต่ระบบบอกไม่ถูกต้อง | เวลาในมือถือหรือใน server คลาดเคลื่อน | ตั้งเวลามือถือเป็นอัตโนมัติ ถ้ายังไม่หายให้แจ้งผู้ดูแลระบบตรวจนาฬิกาเครื่อง server |
| **ทุกคนกรอกเลขไม่ผ่านพร้อมกัน** | นาฬิกาเครื่อง server เพี้ยน | แจ้งผู้ดูแลระบบทันที ดู `docs/mfa-setup.md` |
| เพิ่งกรอกเลขไปแล้วกรอกซ้ำไม่ผ่าน | เลขชุดเดิมใช้ได้ครั้งเดียว | รอชุดถัดไป (ไม่เกิน 30 วินาที) |
| ถูกถามเลขทุกครั้งทั้งที่ติ๊กจำอุปกรณ์ไว้ | เบราว์เซอร์ล้างคุกกี้ หรือใช้โหมดไม่ระบุตัวตน | ติ๊กใหม่อีกครั้ง และเลี่ยงโหมดไม่ระบุตัวตน |
| ลองผิดหลายครั้งแล้วถูกให้รอ | ระบบหน่วงเวลาเพื่อกันการเดารหัส | รอตามเวลาที่แจ้ง แต่ละครั้งที่ผิดจะรอนานขึ้น
