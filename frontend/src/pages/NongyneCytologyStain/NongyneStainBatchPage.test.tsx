...
vi.mock("../../services/nongyneStainService", () => ({
  default: {
...
beforeEach(() => {
  ...
  document
    ...
    .forEach((n) => n.remove());
  mocked(NongyneStainService.getRegisteredQueue).mockResolvedValue([
    item(1, "N26-00001"),
    item(2, "N26-00002"),
  ]);
  mocked(NongyneStainService.createStainRun).mockResolvedValue({ id: 77 });
  mocked(NongyneStainService.printRunStickers).mockResolvedValue(new Blob(["%PDF"]));
...
const renderPage = (onBack = vi.fn()) => {
  render(
    <AntdApp>
      <NongyneStainBatchPage onBack={onBack} />
    </AntdApp>,
  );
  return onBack;
};
...
describe("NongyneStainBatchPage queue", () => {
  ...
  it("says so when the queue cannot be loaded", async () => {
    mocked(NongyneStainService.getRegisteredQueue).mockRejectedValue(new Error("boom"));
    renderPage();
    expect(
  ...
  it("moves a scanned slide out of the queue and into the batch", async () => {
    ...
    await waitFor(() =>
      expect(screen.getAllByText("N26-00001").length).toBe(1),
    );
  });
...
describe("NongyneStainBatchPage starting a run", () => {
  it("does nothing when the batch is empty", async () => {
    renderPage();
    await screen.findByText("N26-00001");

    startStaining();
    expect(NongyneStainService.createStainRun).not.toHaveBeenCalled();
  });
  ...
  it("sends the batched slide ids and reports how many went in", async () => {
    ...
    await waitFor(() =>
      expect(NongyneStainService.createStainRun).toHaveBeenCalledWith(
        "NG-STAINER-01",
        [1, 2],
  ...
  it("reloads the queue so the batched slides do not come back", async () => {
    ...
    startStaining();
    await waitFor(() =>
      expect(mocked(NongyneStainService.getRegisteredQueue).mock.calls.length).toBe(2),
    );
  });
  ...
  it("offers the stickers afterwards and prints only when asked", async () => {
    ...
    await waitFor(() =>
      expect(NongyneStainService.printRunStickers).toHaveBeenCalledWith(77),
    );
  });
  ...
  it("leaves the batch page when the operator defers printing", async () => {
    ...
    expect(NongyneStainService.printRunStickers).not.toHaveBeenCalled();
  });

  it("says so when the run could not be created", async () => {
    mocked(NongyneStainService.createStainRun).mockRejectedValue(new Error("500"));
    renderPage();
    await screen.findByText("N26-00001");
    fireEvent.click(screen.getByText("Move All").closest("button") as Element);
