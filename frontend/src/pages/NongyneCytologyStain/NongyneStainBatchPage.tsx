...
interface NongyneStainBatchPageProps {
  onBack: () => void;
}

const NongyneStainBatchPage: React.FC<NongyneStainBatchPageProps> = ({ onBack }) => {
  const [queue, setQueue] = useState<NongyneCytologyStain[]>([]);
  const [batchItems, setBatchItems] = useState<NongyneCytologyStain[]>([]);
  const [stainerId, setStainerId] = useState("NG-STAINER-01");
  const [loading, setLoading] = useState(false);

  const fetchNewRegisteredQueue = async () => {
    setLoading(true);
    try {
      const data = await NongyneStainService.getRegisteredQueue();
      setQueue(data);
    } catch {
  ...
  const addToBatch = (record: NongyneCytologyStain) => {
  ...
  const handleStartStaining = async () => {
    ...
    try {
      const runName = `NG-${new Date().toISOString().slice(0, 10)}-${new Date().getHours()}${new Date().getMinutes()}`;
      const run = await NongyneStainService.createStainRun(
        stainerId,
  ...
  return (
    <PageContainer withCard title="Create Non-Gyne Staining Batch" onBack={onBack}>
      <Row gutter={24}>
        {/* Left: slides queue */}
        <Col span={11}>
          <Card
            ...
            extra={
              <Space>
                ...
                <Button icon={<ReloadOutlined />} onClick={fetchNewRegisteredQueue} />
              </Space>
            }
          >
            <Search
              ...
              onSearch={(value) => {
                ...
                if (item) addToBatch(item);
                else message.warning("Accession not found in queue");
              }}
          ...
          >
            <Space direction="vertical" style={{ width: "100%" }}>
              ...
              <Table
                ...
                size="small"
                columns={[
                  { title: "No.", render: (_, __, index) => index + 1, width: 50 },
                  { title: "Accession", dataIndex: "accession_no" },
                  {
                    title: "Item",
                    render: (_, record) => (
                      <span>{record.test?.name} (Slide {record.slide_no})</span>
                    ),
                  },
                  {
                    title: "",
                    ...
                    render: (_, record) => (
                      <Button
                        ...
                        icon={<DeleteOutlined />}
                        onClick={() => {
                          setBatchItems(batchItems.filter((i) => i.id !== record.id));
...
export default NongyneStainBatchPage;
