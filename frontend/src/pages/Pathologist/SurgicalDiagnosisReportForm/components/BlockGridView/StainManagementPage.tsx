...
const StainManagementPage: React.FC<StainManagementPageProps> = ({
  ...
  useEffect(() => {
    if (open) {
      ...
      setStaged([]);
      setActiveTab("all");
      setTabSearch({ all: "", ihc: "", special: "", ish: "", molecular: "" });
      setPanelSearch("");
      setTimeline([]);
  ...
  const filteredAll      = stainOrderTests.filter((t) => t.name.toLowerCase().includes(tabSearch.all.toLowerCase()));
  const filteredIhc      = ihcTests.filter((t) => t.name.toLowerCase().includes(tabSearch.ihc.toLowerCase()));
  const filteredSpecial  = specialTests.filter((t) => t.name.toLowerCase().includes(tabSearch.special.toLowerCase()));
  const filteredIsh      = ishTests.filter((t) => t.name.toLowerCase().includes(tabSearch.ish.toLowerCase()));
  const filteredMolecular = molecularTests.filter((t) => t.name.toLowerCase().includes(tabSearch.molecular.toLowerCase()));
  ...
  const toggleStaged = (test: AnatomicalPathologyTest) => {
    ...
    const tabKey =
      test.category === "IHC" ? "ihc" :
      test.category === "Histochem" || test.category === "Special Stain" ? "special" :
      test.category === "ISH" ? "ish" :
      test.category === "Molecular" ? "molecular" :
      null;
    setTabSearch((prev) => ({ ...prev, all: "", ...(tabKey ? { [tabKey]: "" } : {}) }));
  };
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 1050, background: "#fff", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        ...
        <div style={{ width: 340, flexShrink: 0, display: "flex", flexDirection: "column", padding: "16px 20px", borderRight: "1px solid #f0f0f0", overflow: "hidden" }}>
          ...
          {/* Tab content */}
          {activeTab === "all" && <>
            <Search placeholder="Search all tests..." value={tabSearch.all} onChange={(e) => setTabSearch((prev) => ({ ...prev, all: e.target.value }))} size="small" allowClear autoFocus style={{ marginBottom: 6 }} />
          ...
          {activeTab === "ihc" && <>
            <Search placeholder="Search IHC..." value={tabSearch.ihc} onChange={(e) => setTabSearch((prev) => ({ ...prev, ihc: e.target.value }))} size="small" allowClear style={{ marginBottom: 6 }} />
            <div style={{ flex: 1, overflowY: "auto", border: "1px solid #f0f0f0", borderRadius: 6, padding: 4, height: listH }}>
          ...
          {activeTab === "special" && <>
            <Search placeholder="Search Special Stain..." value={tabSearch.special} onChange={(e) => setTabSearch((prev) => ({ ...prev, special: e.target.value }))} size="small" allowClear style={{ marginBottom: 6 }} />
            <div style={{ flex: 1, overflowY: "auto", border: "1px solid #f0f0f0", borderRadius: 6, padding: 4, height: listH }}>
          ...
          {activeTab === "ish" && <>
            <Search placeholder="Search ISH / FISH / CISH..." value={tabSearch.ish} onChange={(e) => setTabSearch((prev) => ({ ...prev, ish: e.target.value }))} size="small" allowClear style={{ marginBottom: 6 }} />
            <div style={{ flex: 1, overflowY: "auto", border: "1px solid #f0f0f0", borderRadius: 6, padding: 4, height: listH }}>
          ...
          {activeTab === "molecular" && <>
            <Search placeholder="Search Molecular..." value={tabSearch.molecular} onChange={(e) => setTabSearch((prev) => ({ ...prev, molecular: e.target.value }))} size="small" allowClear style={{ marginBottom: 6 }} />
            <div style={{ flex: 1, overflowY: "auto", border: "1px solid #f0f0f0", borderRadius: 6, padding: 4, height: listH }}>
          ...
          {activeTab === "panels" && <>
            ...
            <Button size="small" icon={<SettingOutlined />} onClick={() => setIsManagePanelOpen(true)} style={{ marginTop: 6, width: "100%" }}>
              Manage Panels
            </Button>
          </>}
