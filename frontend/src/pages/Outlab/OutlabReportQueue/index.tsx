...
const ReceiveReportModal: React.FC<ReceiveModalProps> = ({ open, caseData, onClose, onSuccess }) => {
  ...
  const handleConfirm = async () => {
    ...
    setUploading(true);
    try {
      await GyneCytologyCaseService.uploadOutlabReport(caseData.id, file);
      message.success("รับรายงาน Outlab สำเร็จ");
      onSuccess();
      onClose();
    } catch (err) {
      logger.error("Upload outlab report error", err);
    ...
    <Modal
      title={
        <Space>
          ...
          <span>Receive Report — {caseData?.accession_no}</span>
        </Space>
      }
      open={open}
      onCancel={onClose}
...
const OutlabReportQueuePage: React.FC = () => {
  ...
  const [receiveModal, setReceiveModal] = useState<{ open: boolean; case: GyneCytologyCase | null }>({ open: false, case: null });
  ...
  const fetchUploaded = useCallback(async (p = 1, s = "") => {
    ...
    try {
      const res = await GyneCytologyCaseService.getAll({ is_out_lab_consult: true, consult_status: "received", skip: (p - 1) * PAGE_SIZE, limit: PAGE_SIZE, search: s || undefined });
  ...
  const handleViewPDF = async (c: GyneCytologyCase) => {
    ...
    try {
      const blob = await GyneCytologyCaseService.downloadOutlabReport(c.id);
      if (pdfUrlRef.current) URL.revokeObjectURL(pdfUrlRef.current);
      const url = URL.createObjectURL(blob);
      pdfUrlRef.current = url;
      setPdfUrl(url);
      setPreviewFilename(`${c.accession_no}_outlab.pdf`);
      setPreviewOpen(true);
  ...
  const patientCol = {
    ...
    render: (c: GyneCytologyCase) => (
      <div>
        <Text strong>{[c.patient?.title?.title, c.patient?.name, c.patient?.ln].filter(Boolean).join(" ") || "-"}</Text>
  ...
  const baseColumns = [
    { title: "Accession No.", dataIndex: "accession_no", key: "accession_no", width: 140, render: (v: string) => <Text strong copyable>{v}</Text> },
    patientCol,
    { title: "Specimen", dataIndex: "specimen_type", key: "specimen_type", render: (v: string) => <Tag color="blue">{v}</Tag> },
    { title: "Registered", dataIndex: "registered_at", key: "registered_at", width: 120, render: (v: string) => dayjs(v).format("DD/MM/YYYY") },
    {
      title: "Days",
  ...
  const pendingColumns = [
    ...baseColumns,
    {
      ...
      width: 160,
      render: (c: GyneCytologyCase) => (
        <Button type="primary" icon={<CloudUploadOutlined />} onClick={() => setReceiveModal({ open: true, case: c })}>
          Receive Report
        </Button>
      ),
    },
    ...
    ...baseColumns,
    {
      ...
      render: (c: GyneCytologyCase) => (
        <Space>
          ...
          <Button icon={<CloudUploadOutlined />} size="small" onClick={() => setReceiveModal({ open: true, case: c })}>
            Re-upload
          </Button>
        </Space>
      ),
  ...
  const searchBar = (
    <Space style={{ marginBottom: 12, display: "flex", justifyContent: "space-between" }}>
      ...
      <Input.Search placeholder="Search HN, Accession No., Name..." allowClear enterButton style={{ width: 300 }}
  ...
  return (
    ...
    >
        ...
        items={[
          {
            ...
            children: (
              <Table dataSource={pendingCases} columns={pendingColumns} rowKey="id" loading={loadingPending} bordered size="middle"
                pagination={{ current: pendingPage, pageSize: PAGE_SIZE, total: pendingTotal, showTotal: t => `Total ${t}`, onChange: p => fetchPending(p, search) }} />
            ),
          },
          {
            key: "uploaded",
            ...
            children: (
              <Table dataSource={uploadedCases} columns={uploadedColumns} rowKey="id" loading={loadingUploaded} bordered size="middle"
                pagination={{ current: uploadedPage, pageSize: PAGE_SIZE, total: uploadedTotal, showTotal: t => `Total ${t}`, onChange: p => fetchUploaded(p, search) }} />
            ),
          },
        ]}
      />
      ...
      <ReceiveReportModal
        ...
        caseData={receiveModal.case}
        onClose={() => setReceiveModal({ open: false, case: null })}
        onSuccess={refresh}
      />
...
export default OutlabReportQueuePage;
