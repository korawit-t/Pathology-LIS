...
const GyneReportHistory: React.FC<Props> = ({ hospital_id }) => {
  ...
  const fetchData = useCallback(async (p: number, search?: string) => {
    setLoading(true);
    try {
      const data = await GyneDiagnosisService.getArchive(p, PAGE_SIZE, search, hospital_id);
      setRows(data.items);
      setTotal(data.total);
    } catch (error) {
  ...
  const handleViewPDF = async (row: ArchiveItem) => {
    ...
    try {
      ...
      setPdfUrl(url);
      setIsModalOpen(true);
      if (row.source === "legacy") legacyReportService.markRead("gyne", row.id).catch(() => {});
    } catch (error) {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "interp",
      render: (_: unknown, r: ArchiveItem) => {
        const text = r.interpretation || r.adequacy_text || r.category_1_text;
        return (
          ...
          expandedRowRender: (r) => (
            <div style={{ background: "#fafafa", padding: 24, borderRadius: 8, border: "1px solid #f0f0f0" }}>
              ...
              <div style={{ background: "#fff", padding: 16, borderRadius: 8, border: "1px solid #f0f0f0" }}>
                <p><b>Adequacy:</b> {r.adequacy_text || "-"}</p>
                <Divider style={{ margin: "12px 0" }} />
                <p><b>Interpretation/Result:</b></p>
                <div style={{ background: "#f9f0ff", padding: 10, borderRadius: 4, border: "1px solid #d3adf7", color: "#391085" }}>
                  {r.interpretation || "-"}
                </div>
                <div style={{ marginTop: 12 }}>
                  <p><b>Bethesda Category:</b></p>
                  {r.category_1_text && <Tag color="magenta">{r.category_1_text}</Tag>}
                </div>
              </div>
            </div>
          ),
