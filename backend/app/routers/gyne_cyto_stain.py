...
@router.get("/registered-queue", response_model=List[GyneStainResponse])
...
@router.get("/pending-print", response_model=List[GyneStainResponse])
...
@router.get("/case/{case_id}", response_model=List[GyneStainResponse])
...
@router.post("", response_model=GyneStainResponse)
def create_new_stain(obj_in: GyneStainCreate, db: Session = Depends(get_db)):
    return crud.create_stain(db, obj_in=obj_in)


# 5. อัปเดตสถานะ (พิมพ์แล้ว/ย้อมแล้ว)
@router.patch("/{stain_id}", response_model=GyneStainResponse)
def update_stain_status(
    stain_id: int, obj_in: GyneStainUpdate, db: Session = Depends(get_db)
):
...
@router.post("/runs")
def create_gyne_stain_run(
    obj_in: GyneStainRunCreate,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),  # 👈 ถ้ามีระบบ Auth
):
    """
    รับรายการ IDs และสร้างตาราง gyne_stain_runs + gyne_stain_run_details
    """
    ...
    # อย่าลืมระบุ user_id ถ้าต้องการเก็บข้อมูล operator_id (เช่นจาก token)
...
def print_gyne_run_stickers(
    ...
    return Response(
        ...
        headers={"Content-Disposition": f"inline; filename=stickers_gyne_{run_id}.pdf"},
    )
